...
# %%
# Function to encapsulate the parallelizable part
def process_combination(relation_method, n_folds, percentile, tree_file, dist_matrix, tip_names, results_folder, results_file, report_dir, models_dict, scoring, prep_pipeline, ana_type, tr):
    ...
    if tr_temp.shape[0] > 100:
        report, top = model_compare_cv(
            X=tr_temp, y=y, preprocess_pipe=prep_pipeline,
            models_dict=models_dict, scoring=scoring,
            report_dir=report_dir, cv=tr_phylo_folds,
            ana_type=ana_type, cache_dir=report_dir
        )
...
for tree_file in os.listdir(tree_folder):
    if tree_file.endswith(".treefile"):
        ...
        # Pre-calculate models_dict and scoring outside the loop
        models_dict = get_models(ana_type=ana_type)
        scoring = get_scores(ana_type=ana_type)

        # Parallelize the combinations
                ...
                for n_folds in n_folds_range:
                    for percentile in percentile_threshold_range:
                        futures.append(executor.submit(
                            process_combination, 
                            relation_method, n_folds, percentile, 
                            tree_file, dist_matrix, tip_names, 
                            results_folder, results_file, report_dir, 
                            models_dict, scoring, prep_pipeline, 
                            ana_type, tr
                        ))

            # Wait for all tasks to complete
            for future in concurrent.futures.as_completed(futures):
                future.result()  # This will raise any exceptions that occurred in the worker processes
