...
from deepBreaks.utils import get_models, get_scores, get_params, get_exp_params, make_pipeline
...
save_to = 'aa_prop_combos_mnm_analysis_2025-03-10_12-48-09'
...
aa_prop_combos_file = f'./{save_to}/top_mnm_aa_props_all_datasets.csv'
aa_prop_combos_df = pd.read_csv(filepath_or_buffer=aa_prop_combos_file,index_col=0)
...
aa_prop_combos_df['gs_r2'] = aa_prop_combos_df['gs_r2'].apply(string_to_numpy_array)
aa_prop_combos_df['gs_params'] = aa_prop_combos_df['gs_params'].apply(string_to_list_of_dicts)

aa_prop_combos_df['best_gs_r2'] = ''
aa_prop_combos_df['best_gs_params'] = ''
aa_prop_combos_df['best_gs_model'] = str('')
for i,row in enumerate(aa_prop_combos_df['gs_r2']):
    best_r2 = max(row)
    best_r2_index = np.argmax(row)
    aa_prop_combos_df.loc[i, ('best_gs_r2')] = best_r2
    
    param_list = aa_prop_combos_df.loc[i, ('gs_params')]
    best_params = param_list[best_r2_index]
    aa_prop_combos_df.loc[i, ('best_gs_params')] = str(best_params)
    
    keys = list(best_params.keys())
    best_model = keys[0].split('_')[0]
    aa_prop_combos_df.loc[i, ('best_gs_model')] = best_model
    
aa_prop_combos_df.to_csv(path_or_buf=f'./{save_to}/top_aa_props_all_datasets_gs.csv', index=True)

# take each dataset, find highest r2 value and then take just that row for the final dataframe
datasets = aa_prop_combos_df['Dataset'].unique()
best_gs_results = []
for ds in datasets:
    ds_df = aa_prop_combos_df[aa_prop_combos_df['Dataset'] == ds]
    ds_df.sort_values('best_gs_r2', ascending=False, inplace=True)
    best_gs_results.append(ds_df.iloc[0])

best_gs_df = pd.DataFrame(data=best_gs_results)
best_gs_df = best_gs_df.reindex(columns=['Dataset','Props_Used','best_gs_model','best_gs_r2','best_gs_params']).reset_index(drop=True)

best_gs_df.to_csv(path_or_buf=f'./{save_to}/filtered_top_aa_props_all_datasets_gs.csv', index=True)