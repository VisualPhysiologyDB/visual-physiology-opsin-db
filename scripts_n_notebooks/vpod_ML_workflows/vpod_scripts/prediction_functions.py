...
def process_sequence(mafft, sequence, selected_model, alignment_data, gap_threshold = 0.5):
...
def process_sequences_from_file(mafft,input_file,output_file,selected_model,alignment_data, gap_threshold = 0.5):
    if input_file == None:
    ...
    with open(output_file, 'w') as f:
        for name, prediction in zip(names, predictions):
            f.write(f"{name}\t{prediction}\n")
            print(f"{name}\t{prediction}\n")
    
    print(f'All Predictions Complete')    
    return(pd.DataFrame({'Name': names, 'Predictions': predictions}))
