...
def finalize_top(X, y, top_models, grid_param, cv, random_state=123, report_dir=None, n_jobs=-1):
    ...
    for model in top_models:
        ...
        print("Hyperparameters retrieved...")
        if len(p_grid) > 0:
            try:
                print("p-grid is greater than 0")
                ...
                print("grid-search set")
                print(grid_param)

                #the grid_search.fit() funtion will take a long-time because we are essentially asking it to find the
                #best combination of hyperparameters within a range - it acheives this by retraining models on each new combination. 
                #so this is a computationally intense section.
                grid_search.fit(X=X, y=y)
                print("Grid-search complete, fit model to best-hyperparameters")
            ...
            print("p-grid is not greater than 0")
            model.fit(X=X, y=y)
            print("fitting model")
            if report_dir is not None:
                joblib.dump(model, report_dir + '/' + name + '.pkl')
